...
export function registerReadTools(
    ...
    server.registerTool(
        ...
        async ({ chainId, method, params }, _extra) => {
            try {
                const { result, batchId } = await client.rpcCall(
                    chainId,
                    method,
                    params,
                );
    ...
    server.registerTool(
        ...
        async (
            { chainId, blockTag, blockHash, includeTransactions },
            _extra,
        ) => {
            try {
                const method = blockHash
                    ? "eth_getBlockByHash"
                    : "eth_getBlockByNumber";
                const param = blockHash ?? normalizeBlockTag(blockTag);
                const { result: block, batchId } = await client.rpcCall(
                    chainId,
                    method,
    ...
    server.registerTool(
        ...
        async ({ chainId, txHash }, _extra) => {
            try {
                const { result: transaction, batchId } = await client.rpcCall(
                    chainId,
    ...
    server.registerTool(
        ...
        async ({ chainId, txHash }, _extra) => {
            try {
                const { result: receipt, batchId } = await client.rpcCall(
                    chainId,
    ...
    server.registerTool(
        "rpc_get_logs",
        {
            ...
            inputSchema: {
                ...
                address: z
                    .union([addressSchema, z.array(addressSchema)])
                    .optional(),
        ...
        async ({ chainId, fromBlock, toBlock, address, topics }, _extra) => {
            try {
                ...
                const { result: logs, batchId } = await client.rpcCall(
                    chainId,
                    "eth_getLogs",
                    [filter],
                );
    ...
    server.registerTool(
        ...
        async ({ chainId, address, blockTag }, _extra) => {
            try {
                const { result: balance, batchId } = await client.rpcCall(
                    chainId,
                    "eth_getBalance",
    ...
    server.registerTool(
        ...
        async ({ chainId, transaction }, _extra) => {
            try {
                const { result: gasEstimate, batchId } = await client.rpcCall(
                    chainId,
                    "eth_estimateGas",
                    [transaction],
                );
    ...
    server.registerTool(
        ...
        async ({ chainId }, _extra) => {
            try {
                const [
                    gasPriceSettled,
                    maxPriorityFeePerGasSettled,
                    feeHistorySettled,
                ] = await Promise.allSettled([
                    client.rpcCall(chainId, "eth_gasPrice", []),
                    client.rpcCall(chainId, "eth_maxPriorityFeePerGas", []),
                    client.rpcCall(chainId, "eth_feeHistory", [
                        "0x1",
                        "latest",
                        [50],
                    ]),
                ...
                const batchIds = {
                    ...
                    maxPriorityFeePerGas:
                        maxPriorityFeePerGasResult?.batchId,
                    feeHistory: feeHistoryResult?.batchId,
                };
                    ...
                    try {
                        ...
                        const res =
                            method === "debug_traceTransaction"
                                ? await client.rpcCall(chainId, method, [
                                      txHash,
                                      {},
                                  ])
                                : await client.rpcCall(chainId, method, [
